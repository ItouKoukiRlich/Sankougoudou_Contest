...
void Player::Control()
{
	...
	if (IsKeyTrigger(VK_SHIFT)) {
		...
		//回避時に押されていた方向に動く
		if (IsKeyPress('W'))	m_Move[Player::MoveDirection::Forward] = 0.1f;
		else if (IsKeyPress('S'))	m_Move[Player::MoveDirection::Forward] = -0.1f;
		else						m_Move[Player::MoveDirection::Forward] = 0.0f;
		if (IsKeyPress('A'))	m_Move[Player::MoveDirection::Left] = 0.1f;
		else if (IsKeyPress('D'))	m_Move[Player::MoveDirection::Left] = -0.1f;
		else						m_Move[Player::MoveDirection::Left] = 0.0f;
		if (IsKeyPress('Q'))	m_Move[Player::MoveDirection::Up] = 0.1f;
		else if (IsKeyPress('E'))	m_Move[Player::MoveDirection::Up] = -0.1f;
		else						m_Move[Player::MoveDirection::Up] = 0.0f;
	...
	if (IsKeyTrigger(VK_SPACE)) {
		for (int i = 0; i < cg_MaxNormalBullet; ++i)
		{
			...
			m_pBullet[i].CreateBullet(vec, Pos);
			break;
		}
	}
